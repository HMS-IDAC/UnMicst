...
if __name__ == '__main__':
	...
	fullPM = np.empty((sizeZ, nClass, fullStack.shape[1], fullStack.shape[2]))
	maxLimit = np.max(fullStack[30, :, :])
	minLimit = np.percentile(fullStack[30, :, :], 10)
	for iZ in range(sizeZ):
		...
		I = im2double(sk.rescale_intensity(I, in_range=(minLimit / 65535, maxLimit / 65535), out_range=(0, 0.983)))
		cells = I
	...
	tifffile.imwrite(
		args.outputPath + '//' + fileNamePrefix[0] + '_Probabilities1-5_' + str(int(dapiChannel) + 1) + '.tif',
		np.uint8(255 * fullPM), imagej=True)
