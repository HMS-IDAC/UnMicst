    ...
    def setup(imSize, nChannels, nClasses, nOut0, featMapsFact, downSampFact, kernelSize, nExtraConvs, stdDev0,
        ...
        with tf.variable_scope('lb'):
            ...
            def lb(hidden):
                ...
                return tf.layers.dropout(lbn, 0.05, training=UNet2D.tfTraining)
        ...
        def up_samp_layer(data, index):
            with tf.variable_scope('lu%d' % index):
                regularizer = tf.keras.regularizers.l2(0.00000005)
                ...
                for i in range(nExtraConvs):
                    cv = tf.nn.leaky_relu(tf.nn.conv2d(cv, luXWeightsExtra[i], strides=[1, 1, 1, 1], padding='SAME'),
                                          name='conv2Extra%d' % i)
                # cv = tf.layers.dropout(cv, 0.15 - 0.05 * index, training=UNet2D.tfTraining)
                return cv
    ...
    def train(imPath, validPath, testPath, logPath, modelPath, pmPath, nTrain, nValid, nTest, restoreVariables, nSteps, gpuIndex,
        ...
        for iSample in range(0, nTrain):
            ...
            for iChan in range(nChannels):
                ...
                Train[iSample, :, :, iChan] = (im - datasetMean) / datasetStDev
            ...
            for i in range(nClasses):
                ...
                if i == 0:
                    WTrain[iSample, :, :, i] = ((im==1) * nucleiWeight)
                if i == 1:
                    WTrain[iSample, :, :, i] = ((im==2) * cytoWeight)
                if i == 2:
                    WTrain[iSample, :, :, i] = ((W==1) * bgWeight)
        ...
        for iSample in range(0, nValid):
            ...
            for iChan in range(nChannels):
                im = im2double(skio.imread(path, img_num=iChan))
                Valid[iSample, :, :, iChan] = (im - datasetMean) / datasetStDev
            ...
            for i in range(nClasses):
                ...
                if i == 0:
                    WValid[iSample, :, :, i] = ((im==1) * nucleiWeight)
                if i == 1:
                    WValid[iSample, :, :, i] = ((im==2) * cytoWeight)
                if i == 2:
                    WValid[iSample, :, :, i] = ((W==1) * bgWeight)
        ...
        for iSample in range(0, nTest):
            ...
            for iChan in range(nChannels):
                im = im2double(skio.imread(path, img_num=iChan))
                Test[iSample, :, :, iChan] = (im - datasetMean) / datasetStDev
            ...
            for i in range(nClasses):
                ...
                if i == 0:
                    WTest[iSample, :, :, i] = ((im==1) * nucleiWeight)
                if i == 1:
                    WTest[iSample, :, :, i] = ((im==2) * cytoWeight)
                if i == 2:
                    WTest[iSample, :, :, i] = ((W==1) * bgWeight)
        ...
        with tf.name_scope('images'):
            ...
            split4= tf.slice(UNet2D.tfData, [0, 0, 0, 1], [-1, -1, -1, 1])
            ...
            planeImN1 = tf.div(tf.subtract(split4, tf.reduce_min(split4, axis=(1, 2), keep_dims=True)),
                              tf.subtract(tf.reduce_max(split4, axis=(1, 2), keep_dims=True),
                                          tf.reduce_min(split4, axis=(1, 2), keep_dims=True)))

            plane = tf.concat([planeImN, planeImN1, split0], 2)
            split2 = tf.slice(UNet2D.nn, [0, 0, 0, 1], [-1, -1, -1, 1])
            split3 = tf.slice(UNet2D.nn, [0, 0, 0, 2], [-1, -1, -1, 1])
            ...
            plane = tf.concat([plane, split2, split3], 2)
        ...
        for i in range(nSteps):
            ...
            for j in range(batchSize):
                ...
                if np.random.rand() < 0.25:
                    ...
                    batchLabels[j, :, :, :] = np.flipud(LTrain[permT[jT + j], :, :, :])
                    batchWeights[j, :, :, :] = np.flipud(WTrain[permT[jT + j], :, :, :])
                # 	image[:,:,1] = image
                batchData[j, :, :, :] =  image
...
if __name__ == '__main__':
    logPath = 'G:\\3Dseg\\TFLogs'
    modelPath = 'G:\\3Dseg'
    pmPath = 'G:\\3Dseg\\TFProbMaps'

    # ----- test 1 -----

    # imPath = 'F:\\3Dseg\\trainingPatches'
    # validPath = 'F:\\3Dseg\\trainingPatches\\valid'
    # testPath = 'F:\\3Dseg\\trainingPatches\\test'
    #
    # UNet2D.setup(256, 2, 3, 80, 2, 2, 3, 0, 0.03, 4, 4)  # 64 0.0001 4
    # UNet2D.train(imPath, validPath, testPath, logPath, modelPath, pmPath, 759, 388, 68, False, 5000, 0, 2)
    # UNet2D.deploy(testPath, 243, modelPath, pmPath, 0, 1)
    ...
    modelPath = 'G:/3Dseg'#os.path.join(scriptPath, 'models', args.model)
    ...
    fullStack = tifffile.imread(imagePath)
    sizeZ = fullStack.shape[0]
    fullPM = np.empty((sizeZ,nClass,fullStack.shape[2],fullStack.shape[3]))
    maxLimit = np.max(fullStack[:,0,:,:])
    minLimit = np.percentile(fullStack[:,0,:,:],2)
    fullStack = im2double(sk.rescale_intensity(fullStack, in_range=(minLimit, maxLimit), out_range=(0, 1)))
    for iZ in range(sizeZ):
        print(str(iZ))
        for iChan in range(len(channel)):
            I = fullStack[iZ,iChan,:,:]
            if I.dtype == 'float32':
                I = np.uint16(I)
            rawVert = I.shape[0]
            rawHorz = I.shape[1]
        ...
        slice=0
        for iClass in range(0,2):#args.classOrder[::-1]:
            ...
            fullPM[iZ,iClass,:,:]=PM
        # if slice==0:
        #     skimage.io.imsave(args.outputPath + '//' + fileNamePrefix[0] + '_Probabilities_' + str(int(dapiChannel)+1) + '_Z_' + str(iZ) + '.tif', np.uint8(255 * PM),**save_kwargs)
        # else:
        #     skimage.io.imsave(args.outputPath + '//' + fileNamePrefix[0] + '_Probabilities_' + str(int(dapiChannel)+1) + '_Z_' + str(iZ) + '.tif',np.uint8(255 * PM),**append_kwargs)
        # if slice==1:
    ...
    tifffile.imwrite(
        args.outputPath + '//' + fileNamePrefix[0] + '_Probabilities_' + str(int(dapiChannel) + 1) + '.tif', np.uint8(255 * fullPM), imagej=True)
