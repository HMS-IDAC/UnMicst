...
class UNet2D:
	...
	def setup(imSize, nChannels, nClasses, nOut0, featMapsFact, downSampFact, kernelSize, nExtraConvs, stdDev0,
		...
		with tf.variable_scope('lb'):
			...
			def lb(hidden):
				...
				return tf.layers.dropout(lbn, 0.2, training=UNet2D.tfTraining)
	...
	def train(imPath, validPath, testPath, logPath, modelPath, pmPath, nTrain, nValid, nTest, restoreVariables, nSteps, gpuIndex,
		...
		# 	I = im2double(skio.imread('%s/I%05d_Img.tif' % (imPath,iSample)))
		...
		for iSample in range(0, nTrain):
			...
			for i in range(nClasses):
				...
				if i == 0:
					...
					WTrain[iSample, :, :, i] = (W * intersectWeight) + bgWeight
		...
		for iSample in range(0, nValid):
			...
			for i in range(nClasses):
				...
				if i == 0:
					...
					WValid[iSample, :, :, i] =  (W * intersectWeight)  + bgWeight
		...
		for iSample in range(0, nTest):
			...
			for i in range(nClasses):
				...
				if i == 0:
					...
					WTest[iSample, :, :, i] = (W * intersectWeight) + bgWeight
	...
	def singleImageInference(image, mode, pmIndex):
		...
		for i in range(PI2D.NumPatches):
			...
			for iChan in range(nChannels):
				batchData[j, :, :, iChan] = (PI2D.getPatch(i) - UNet2D.DatasetMean) / UNet2D.DatasetStDev
...
if __name__ == '__main__':
	# logPath = 'D:\\Dan\\CytoplasmZeissNikon11\\TFLogs'
	# modelPath = 'D:\\Dan\\CytoplasmZeissNikon11'
	# pmPath = 'D:\\Dan\\CytoplasmZeissNikon11\\TFProbMaps'

# ----- test 1 -----

# imPath = 'D:\\Seidman\\IncellTrainingSet10'
# validPath = 'D:\\Seidman\\IncellTrainingSet10\\valid'
# testPath = 'D:\\Seidman\\IncellTrainingSet10\\test'
# UNet2D.setup(256, 1, 2, 30, 2, 2, 3, 0, 0.007, 3, 16)
# UNet2D.train(imPath, validPath, testPath, logPath, modelPath, pmPath, 993, 78, 24, False, 0, 1, 1)
# UNet2D.deploy(testPath, 24, modelPath, pmPath, 1, 0)
	#
	...
	if args.stackOutput:
		...
		for iClass in args.classOrder[::-1]:
			PM = np.uint8(255 * UNet2D.singleImageInference(I, 'accumulate',
